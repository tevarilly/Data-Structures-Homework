...
Tetris::Tetris(int w)
{
	...
	//Initialize max height to be zero: will be modified as we go
	max_height=0;
}
...
void Tetris::add_piece(char shape, int rot, int loc)
{
	...
	for(int i=loc;i<loc+shape_width;i++)
	{
		...
		std::cout<<"Allocate "<<new_height<<" to column "<<i<<"\n";
	...
	for(int i=0,l=loc,h=old_height;i<shape_width;i++)
	{
		...
		{
			std::cout<<"write "<<shape_array[j][i]<<" to "<<l<<","<<h<<" from "<<i<<","<<j<<"\n";
...
int Tetris::get_biggest(int begin, int end)
{
	int big = 0;
