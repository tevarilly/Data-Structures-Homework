...
public:
  Node();
  Node(const Point<number_type> pt_, const label_type label_, const Node<number_type,label_type>* parent_) : pt(pt_),label(label_),parent(parent_) {}
  // REPRESENTATION
...
void QuadTree::plot(int max_x, int max_y, bool draw_lines=true) const {
...
void QuadTree::plot(Node<number_type,label_type> *p, std::vector<std::string> &board,
